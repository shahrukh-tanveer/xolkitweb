...
const services = [
  {
    ...
    href: "/services/customer-center",
    description:
      "Build scalable customer support centers powered by AI and humans.",
    Icon: ChatBubbleLeftRightIcon,
  },
  {
    name: "Web Development",
    href: "/services/web-development",
    description: "Modern, performant websites and web apps.",
    Icon: GlobeAltIcon,
  },
  {
    name: "Mobile App Development",
    href: "/services/mobile-app-development",
    description: "iOS and Android apps with delightful UX.",
    Icon: DevicePhoneMobileIcon,
  },
  {
    name: "SaaS",
    href: "/services/saas",
    description: "End-to-end SaaS product design, build, and launch.",
    Icon: CloudIcon,
  },
  {
  ...
  },
  {
    ...
    description:
      "Autonomous and assistive agents that integrate with your stack.",
    Icon: SparklesIcon,
  },
  {
    name: "Graphic Design",
    href: "/services/graphic-design",
    description: "Branding, UI, and visual assets that communicate clearly.",
    Icon: PaintBrushIcon,
  },
];
...
const portfolio = [
  {
    ...
    description: "Marks, wordmarks, and brand systems.",
    Icon: SwatchIcon,
  },
  {
    name: "Web Development",
    href: "/portfolio/web-development",
    description: "Web projects and case studies.",
    Icon: GlobeAltIcon,
  },
  {
    name: "App Development",
    href: "/portfolio/app-development",
    description: "Mobile and desktop apps.",
    Icon: DevicePhoneMobileIcon,
  },
];
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-colors duration-500 ${
        solid ? "navbar-solid" : "navbar-initial"
      } ${shrink ? "navbar-shrink" : ""} ${
        justActivated ? "navbar-solid-enter" : ""
      }`}
    >
      <nav className="container-px">
        <div className="nav-inner flex items-center justify-between h-16 md:h-20">
                  ...
                  {services.map((item) => (
                    <Menu.Item key={item.href}>
                      {({ active }) => (
                        <Link
                          href={item.href}
                          className={`group/item relative flex flex-col gap-2 rounded-xl p-4 w-[220px] flex-shrink-0 ${
                            active ? "bg-[#16212c]" : "bg-[#101c26]"
                          } hover:bg-[#1b2732] hover:border-slate-400/40 transition`}
                        >
                  ...
                  {portfolio.map((item) => (
                    <Menu.Item key={item.href}>
                      {({ active }) => (
                        <Link
                          href={item.href}
                          className={`group/item relative flex gap-3 rounded-xl p-4  ${
                            active ? "bg-[#16212c]" : "bg-[#101c26]"
                          } hover:bg-[#1b2732] hover:border-slate-400/40 transition`}
                        >
                          <span className="inline-flex h-10 w-10 items-center justify-center rounded-lg text-slate-200 ">
                            <item.Icon className="h-5 w-5" />
                          </span>
                    ...
                    <Disclosure.Panel className="pl-3 grid grid-cols-1 gap-1">
                      {services.map((s) => (
                        <Link
                          key={s.href}
                          href={s.href}
                          className="flex items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-[#1b2732] border border-transparent hover:border-slate-400/40 transition"
                        >
                          {s.Icon && (
